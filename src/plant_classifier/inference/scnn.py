...
class TwoStageSiamesePredictor:
    ...
    def from_artifacts(
        ...
        require_two_stage_reference_index: bool = False,
        **kwargs,
        ...
        genus_references, species_references = load_two_stage_reference_index(
            reference_index,
            map_location=device,
            require_two_stage=require_two_stage_reference_index,
        )
        return cls(
...
def load_two_stage_reference_index(
    ...
    if require_two_stage:
        raise ValueError(
            "Reference index does not contain separate genus and species references. "
            "Rebuild it with the current plant-classifier-build-index command."
        )
