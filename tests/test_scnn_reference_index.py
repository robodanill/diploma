...
def test_legacy_reference_index_can_be_rejected_for_final_app_profile(tmp_path: Path) -> None:
    output_path = tmp_path / "legacy_reference_index.pt"
    save_reference_index([_reference("legacy.jpg", "Acer", "Acer alpha")], output_path)

    with pytest.raises(ValueError, match="separate genus and species references"):
        load_two_stage_reference_index(output_path, require_two_stage=True)
